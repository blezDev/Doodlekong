...
class ImageRadioButton @JvmOverloads constructor(context : Context, attrs : AttributeSet?=null) : AppCompatRadioButton(context,attrs) {
    ...
    override fun onSizeChanged(w: Int, h: Int, oldw: Int, oldh: Int) {//called when size of view is changed or it is initialized for the first time
    ...
    override fun onDraw(canvas: Canvas?) {
        ...
        canvas?.let {
            if (!isChecked){
                uncheckedDrawable?.setBounds(paddingLeft,
                    paddingTop,
            ...
            {
                checkedDrawable?.setBounds(paddingLeft,
                    paddingTop,
                    viewWidth - paddingRight,
                    viewHeight - paddingBottom )
